...
public static class ValidationExtensions
{
    public static ValidationResult? Validate(this CaseData caseData)
    {
        ...
        {
            if (!caseData.MailingAddress.ContainsKey("mailingStreet") || string.IsNullOrWhiteSpace(caseData.MailingAddress["mailingStreet"]))
            ...
            if (!caseData.MailingAddress.ContainsKey("mailingCity") || string.IsNullOrWhiteSpace(caseData.MailingAddress["mailingCity"]))
            ...
            if (!caseData.MailingAddress.ContainsKey("mailingState") || string.IsNullOrWhiteSpace(caseData.MailingAddress["mailingState"]) ||
                !Regex.IsMatch(caseData.MailingAddress["mailingState"], @"^[A-Z]{2}$"))
                results.Add(new ValidationResult("MailingState must be a valid 2-letter code.", new[] { nameof(caseData.MailingAddress) }));

            if (!caseData.MailingAddress.ContainsKey("mailingZip") || string.IsNullOrWhiteSpace(caseData.MailingAddress["mailingZip"]) ||
                !Regex.IsMatch(caseData.MailingAddress["mailingZip"], @"^\d{5}(-\d{4})?$"))
